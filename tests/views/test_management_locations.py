...
@pytest.mark.django_db
def test_location_add_from_company_view_unauthenticated_access(client: Client, company1):
    """
    Test that a visitor cannot access the view without authentication
    """
...
@pytest.mark.django_db
def test_location_add_from_company_view_unauthorized_access(client: Client, user1, company1):
    """
    Test that the view is unavailable if user does not have proper rights to the stock location's location
    """
    client.force_login(user1)
...
@pytest.mark.django_db
def test_location_add_from_company_view_authenticated_access(client: Client, user2, company1):
    """
    Test that the view is unavailable if user does not have proper rights to the stock location's location
    """
    client.force_login(user2)
